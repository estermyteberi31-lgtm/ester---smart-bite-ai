    ...
    <div className="flex flex-col gap-4">
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <div className="flex items-center gap-2 rounded-full bg-white/5 p-1 text-[11px]">
            <button
              ...
              className={`rounded-full px-2.5 py-1 font-medium transition-colors ${
                gymMode === "home" ? "bg-purple-600 text-white" : "text-white/50"
              }`}
            >
              Home
            </button>
            <button
              ...
              className={`rounded-full px-2.5 py-1 font-medium transition-colors ${
                gymMode === "gym" ? "bg-purple-600 text-white" : "text-white/50"
              }`}
            >
              Gym
            </button>
        ...
        <button
          ...
          className="mt-4 flex h-44 w-full flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-white/15 bg-black/20 text-white/60 transition-colors active:border-purple-500"
        >
          {previewUrl ? (
            <img src={previewUrl} alt="Selected capture" className="h-full w-full rounded-xl object-cover" />
          ) : (
            <>
              <svg viewBox="0 0 24 24" fill="none" className="h-9 w-9 text-purple-400">
                <path
        ...
        <textarea
          ...
          rows={2}
          className="mt-3 w-full resize-none rounded-xl border border-white/10 bg-black/20 p-3 text-sm placeholder:text-white/30 focus:border-purple-500 focus:outline-none"
        />
        ...
        <button
          ...
          onClick={handleScan}
          disabled={loading}
          className="mt-3 w-full rounded-xl bg-gradient-to-r from-purple-600 to-fuchsia-600 py-3 text-sm font-semibold shadow-lg shadow-purple-900/40 transition-opacity disabled:opacity-50"
        >
  ...
  return (
    <div className="flex flex-col gap-4">
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
        ...
        {result.insight && (
          <p className="mt-3 rounded-lg bg-purple-500/10 p-3 text-xs text-purple-200">{result.insight}</p>
        )}
      </section>
  ...
  return (
    <div className="mt-4">
      <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-black/30">
        <div className="bg-purple-500" style={{ width: `${(protein / total) * 100}%` }} />
